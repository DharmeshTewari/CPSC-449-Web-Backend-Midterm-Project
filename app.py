...
def login():
    ...
    if loginId and password:
        ...
        if account:
            access_token = encode_token(loginId)
        ...
        if user:
            ...
            if fs and allowed_file(fs.filename):
                ...
                if file_length < ALLOWED_FILESIZE:
                    ...
                    return "File is saved and uploaded"
                else:
                    return jsonify({'error':"File uploaded is too large file. Upload a smaller file."}), 400
            else:
...
def encode_token(loginId):
    return jwt.encode({"loginId":loginId},app.config['SECRET_KEY'],'HS256')
